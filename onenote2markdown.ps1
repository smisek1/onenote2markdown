...
function handlePage($page, $path, $assets_path, $i) {
    ...
    if (-Not(Test-Path $path_md)) {
        ...
        pandoc.exe -i $path_docx -o $path_md -t markdown-simple_tables-multiline_tables-grid_tables --wrap=none --extract-media=$path_assets
        pandoc.exe -i $path_docx -o $path_md -t markdown-simple_tables-multiline_tables-grid_tables --wrap=none
        #pandoc.exe -i $path_docx -o $path_html -t html --extract-media=$path_assets

        rm $path_docx
        #rm $path_htm
    }
    if (Test-Path $path_assets) {
        $path_assets_wild = Join-Path $path_assets "media\*"
        ...
        # rmdir $path_assets
        Remove-Item -Path $path_assets -Recurse -Force
        ...
        $hello = (Get-Content -encoding utf8 $path_md) `
            -replace $LINK_REGEX, ("![[$prefix" + "image" + ('$2'.TrimStart('0')) + '.$3]]') `
            -replace 'jpeg', 'jpg' |
